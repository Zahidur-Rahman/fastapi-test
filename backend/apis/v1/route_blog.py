...
from db.repositary.blog import create_new_blog,retreive_blog,list_blogs,update_blog_by_id,delete_blog_by_id
...
def get_blog(id:int ,db:Session=Depends(get_db)):
    ...
    if not blog:
        raise HTTPException(detail=(f"Blog with id {id} not in the Database"),status_code=status.HTTP_404_NOT_FOUND)
    return blog
...
def delete_a_blog(id:int ,db:Session=Depends(get_db),current_user:User=Depends(get_current_user)):
    message=delete_blog_by_id(id=id,db=db,author_id=current_user)
